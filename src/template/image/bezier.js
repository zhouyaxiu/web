...
function Draw (ctx, pointset, strokestyle, fillstyle, radius, linewidth, num, colorpt, scalei) {
  ...
  for (var i = 0; i < pointset.length; i = i + 3) {
    ...
    if ((num > -1 && Math.abs(i - num) < 2) || (i === 0 && num === pointset.length - 1 && pointset.length % 3 === 0)) {
      ...
      if (i - 1 > 0) {
        ...
        ctx.arc(pointset[i - 1].x * scalei, pointset[i - 1].y * scalei, radius, 0, 2 * Math.PI)
        ctx.stroke()
        ctx.fill()
      }
      // 闭合曲线的首点
      if (ptnum === 0 && pointset.length % 3 === 0) {
        ...
        ctx.fillStyle = colorpt
        ctx.strokeStyle = colorpt
        ctx.arc(pointset[pointset.length - 1].x * scalei, pointset[pointset.length - 1].y * scalei, radius, 0, 2 * Math.PI)
        ctx.stroke()
        ctx.fill()
      }
  ...
  if (ptnum > -1) {
    ...
    ctx.setLineDash([4, 2])
    if (ptnum - 1 > -1) {
      ctx.moveTo(pointset[ptnum].x * scalei, pointset[ptnum].y * scalei)
      ctx.lineTo(pointset[ptnum - 1].x * scalei, pointset[ptnum - 1].y * scalei)
    } else if (pointset.length % 3 === 0) {
      ctx.moveTo(pointset[0].x * scalei, pointset[0].y * scalei)
      ctx.lineTo(pointset[pointset.length - 1].x * scalei, pointset[pointset.length - 1].y * scalei)
    }
  ...
  for (i = 0; i < pointset.length - 3; i = i + 3) {
    ctx.bezierCurveTo(pointset[i + 1].x * scalei, pointset[i + 1].y * scalei, pointset[i + 2].x * scalei, pointset[i + 2].y * scalei, pointset[i + 3].x * scalei, pointset[i + 3].y * scalei)
  }
  if (i === pointset.length - 3) {
    ctx.bezierCurveTo(pointset[i + 1].x * scalei, pointset[i + 1].y * scalei, pointset[i + 2].x * scalei, pointset[i + 2].y * scalei, pointset[0].x * scalei, pointset[0].y * scalei)
  }
  ctx.stroke()
  ctx.fill()
}
