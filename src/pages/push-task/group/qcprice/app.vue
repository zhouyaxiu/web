<template>
  <div id="main" class="main push-task">
    <Header/>
    <Layout :selStep="4" :active="4" :type="'group'" :projectId="projectId" style="flex:1">
          ...
          <el-row :gutter="20">
            <el-col :span="14">
              <el-form-item :label="$t('pushimage.qcprice.unitprice')" prop="offer">
                ...
                <span class="ml-2">{{$t("pushimage.qcprice.inter")}}</span>
              </el-form-item>
            </el-col>
            <el-col :span="10">
              <div class="alert alert-success" role="alert" style="font-size: .875rem">
                <p class="mb-1">
                  注:1元=100{{$t("pushimage.qcprice.inter")}}
                </p>
                <p class="mb-0">
  ...
  methods: {
    submitForm (formName, forword) {
      ...
      this.$refs[formName].validate(valid => {
        ...
        axios
          .post(
            '/api/push-project-qcprice/group',
            {
              taskid: vm.projectId,
    ...
    goBack () {
      util.Redirect('/push-project-business/group/' + this.projectId, 1000)
    }
  }
}
</script>
