  ...
  <div class="container mb-5">
    ...
    <div class="check-list">
      <el-form ref="query" :model="query" :inline="true">
        ...
        <el-form-item>
          <el-input v-model="query.suser" clearable placeholder="提交人" disabled></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <el-table :data="tasks" border="" v-loading="loading" v-if="tasks.length > 0">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary" size="small" @click="check(scope.row)">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  data () {
    return {
      ...
      tasks: [
        // {
        //   checkuser: 'test-wujianglin',
        //   status: 1001999,
        //   done: 999,
        //   taskid: '167SDSZX001',
        //   checkres: 1001,
        //   username: 'zhao19970307',
        //   uuid: 'OPjwtJ4IYZdPcFF3aeSfJtysAX1hPbVZ'
        // }, {
        //   checkuser: 'test-wujianglin',
        //   status: 1001999,
        //   done: 0,
  ...
  methods: {
    ...
    getTasks (pageNum = 1) {
      ...
      let url = '/api/projects/' + projectId + '/task/revises'
      ...
      http.post(url, {
        ...
        .then(function (response) {
          ...
          if (rsp.code === 0) {
            vm.pager = rsp.pager
            vm.tasks = rsp.tasks
            vm.query.suser = rsp.tasks[0].username
            vm.loading = false
          } else {
            vm.loading = false
            vm.$notify.error({
    ...
    check (row) {
      let url = '/projects/' + projectId + '/task/revise?' + 'taskid=' + row.taskid + '&uuid=' + row.uuid + '&status=' + row.status
      window.open(url)
