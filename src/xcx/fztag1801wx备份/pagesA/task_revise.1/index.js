...
Page({
  ...
  getTaskInfo() {
    cfg.wxPromise(wx.request)({
      url:cfg.getAPIURL() + "/api/projects/" + this.data.taskid + "/task/review?"+ 'taskid='+this.data.query.taskid+'&uuid='+this.data.query.uuid+'&status='+this.data.query.status,
      header : app.globalData.header,
    })
    .then(response=>{
      if (response.data.code === 0) {
